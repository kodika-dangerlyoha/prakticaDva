...
function makeFavorite_favorite() {
    if (favorite_list.length > 0) {
        document.getElementById('favorite_nothing').remove();
        document.querySelector('#favorite #favorite_grid').innerHTML += favorite_list.reduce((summ_game, game) => summ_game + get_horizont_game_html(
            game,  
